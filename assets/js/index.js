...
function saveNewCustomer(){
    ...
    localStorage.setItem("cxName", customerNameField.value); //this works
}
...
//1. create the table
//2. create
let saveCustomerButton = document.getElementById('saveCustomer');
let cxNameInput = document.getElementById('customerName');
let theCustomerForm = document.getElementById('newCustomer');


//empty array below
let customerStorage = localStorage.getItem("cxName")
    ? JSON.parse(localStorage.getItem("cxName"))
    : [];
console.log(customerStorage);
  customerStorage.forEach(cx);
  function storeCx(cxNameInput) {
    const li = document.createElement('li');
    li.textContent = cxNameInput;
    ul.appendChild(li);
  };

  function add(){
    customerStorage.push(cxNameInput.value);
    console.log(customerStorage);
    localStorage.setItem('cxName', JSON.stringify(customerStorage));
    storeCx(cxNameInput.value);
    input.value = '';
  }
  function del(){
    localStorage.clear();
...
saveCustomerButton.addEventListener('submit', (e) =>{
  e.preventDefault();
  //customerStorage.push(cxNameInput.value);
add();
  //localStorage.setItem('cxName', JSON.stringify(cxNameInput.value));
  //listBuilder(cxNameInput.value);
});
...
let createdInvoice = localStorage.getItem("invoice")
  ...
  newInvoiceForm.addEventListener("submit", (e) => {
    ...
    localStorage.setItem("userList", JSON.stringify(createdInvoice));
...
//****************************************************** */
// //*********************** use this code to create a new invoice file for each 
// //9-28 mini project C:\Users\matte\Desktop\uncc\UNCC-VIRT-FSF-FT-08-2023-U-LOLC\09-NodeJS\01-Activities\28-Stu_Mini-Project
...
// //, htmlpagecontent, (err) => err ? console.log
...
// let generateHtml= ({name, location, github}) =>
// `<!DOCTYPE html>
// <html lang="en">
// <head>
//     <meta charset="UTF-8">
